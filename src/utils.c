...
void validate_args(cl_args *args, int argc, char *argv[])
{
    ...
    while ((ch = getopt_long(argc, argv, "b:f:d:h", options, NULL)) != -1)
    { // Defining the arguments
        ...
        case 'b':                             // For buffer
            ...
            if (args->buffer_size < MIN_BUFFER_SIZE)
            {                       // If the buffer size < MIN_BUFFER_SIZE then it needs to change to a higher value
        ...
        case 'd': // For drive
            if (!method_selected)
            {                                                // Stripping the filename of any whitespace
...
void rstrip(char string[])
{
    ...
    for (int n = strlen(string), i = n - 1; i >= 0; i--)
    {
        ...
        {
            break;
        } // Breaks when it first finds a non-whitespace character from the right
    }
}
...
void lstrip(char string[])
{
    ...
    for (int i = 0; i < length; i++)
    {
        ...
        {
            break;
        } // Breaks when it finds the first non-whitespace character
    }
